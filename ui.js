...
document.addEventListener('DOMContentLoaded', () => {
    const hintButton = document.getElementById('hint-button');
    if (hintButton) {
        // hintButton.addEventListener('click', useHint); // 이 줄은 game.js로 이동
    }
}); // 힌트 버튼 이벤트 리스너 추가
...
// 초기 게임 로드 시 힌트 횟수 업데이트
document.addEventListener('DOMContentLoaded', updateHintDisplay);
...
function renderCards(cards, newCardId) {
    ...
    cards.forEach(card => {
        ...
        cardContainer.addEventListener('touchstart', (e) => {
            // e.preventDefault(); // 기본 동작(확대 등) 방지, 필요시 주석 해제
            pressTimer = setTimeout(() => {
                isLongPress = true;
                handleCardDiscard(card.id);
                // 시각적 피드백 추가 가능 (예: 카드 테두리 변경)
            }, 2000); // 2초
        });
        ...
        cardContainer.addEventListener('touchend', () => {
            ...
            if (isLongPress) {
                isLongPress = false;
                // 롱프레스 후 클릭 이벤트가 발생하지 않도록 방지
                // (이벤트 버블링을 막거나, 플래그를 사용하여 click 핸들러에서 처리)
            }
        });
        ...
        let preventClick = false; // 전역 플래그: 롱프레스 후 클릭 방지
